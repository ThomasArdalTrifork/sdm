...
public class FileSpoolerSetupTest {
	...
	public void runSetupTest() throws Exception {
		...
		FileSpoolerSetup setup = new FileSpoolerSetup("TestSpooler", spoolerDir,
				FakeFileImporter.class);

		Assert.assertEquals(setup.rootDir, spoolerDir + "/TestSpooler");

		Assert.assertEquals(setup.getInputPath(), spoolerDir + "/TestSpooler/"
				+ FileSpoolerSetup.INPUT_DIR);

		Assert.assertEquals(setup.getRejectPath(), spoolerDir + "/TestSpooler/"
				+ FileSpoolerSetup.REJECT_DIR);

		Assert.assertEquals(setup.getProcessingPath(), spoolerDir + "/TestSpooler/"
				+ FileSpoolerSetup.PROCESSING_DIR);

		Assert.assertEquals(setup.getStableSeconds(), FileSpoolerSetup.DEFAULT_STABLE_SECONDS);
		Assert.assertEquals(setup.getImporterClass(), FakeFileImporter.class);
