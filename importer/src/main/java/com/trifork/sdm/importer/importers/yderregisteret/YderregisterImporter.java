...
public class YderregisterImporter implements FileImporterControlledIntervals
{
	...
	private static final String[] requiredFileExt = new String[] { "K05", "K40", "K45", "K1025",
			"K5094" };
	...
	public void importFiles(List<File> files) throws FileImporterException
	{
		...
		{
			throw new FileImporterException(
					"Der blev ikke fundet yderregister filer med et løbenummer");
		}
		...
		{
			logger.error(
					"An error occured while persisting the yderregister to database "
							+ e.getMessage(), e);
			throw new FilePersistException(
					"An error occured while persisting the yderregister to database: "
							+ e.getMessage(), e);
