...
public class CPRIntegrationTest {
	...
	public void cleanDatabase() throws Exception {
		...
		statement.execute("truncate table " + MySQLConnectionManager.getHousekeepingDBName()
				+ ".AdresseBeskyttelse");
		statement.close();
		con.close();
	}
	...
	public void ImportPersonNavnebeskyttelsesTest() throws Exception {
		...
		assertEquals(CPRParser.yyyy_MM_dd.parse("1997-09-09"),
				rs.getDate("NavneBeskyttelseStartDato"));
		assertEquals(CPRParser.yyyy_MM_dd.parse("2001-02-20"),
				rs.getDate("NavneBeskyttelseSletteDato"));
	...
	public void ImportForaeldreMyndighedBarnTest() throws Exception {
		...
		rs = stmt
				.executeQuery("Select * from ForaeldreMyndighedRelation where CPR='3112970028' order by TypeKode");
		rs.next();
	...
	public void ImportUmyndighedVaergeTest() throws Exception {
		...
		Statement stmt = con.createStatement();
		ResultSet rs = stmt
				.executeQuery("Select * from UmyndiggoerelseVaergeRelation where CPR='0709614126'");
		rs.next();
	...
	public void ImportU12160Test() throws Exception {
		...
		rs = stmt
				.executeQuery("Select count(*) from Person WHERE NavneBeskyttelseStartDato < now() AND NavneBeskyttelseSletteDato > now()");
		rs.next();
		assertEquals(1, rs.getInt(1));
		...
		rs = stmt.executeQuery("Select count(*) from "
				+ MySQLConnectionManager.getHousekeepingDBName() + ".AdresseBeskyttelse");
		rs.next();
		assertEquals(1, rs.getInt(1));
		stmt.close();
		con.close();
	...
	public void UpdateTest() throws Exception {
		...
		Statement stmt = con.createStatement();
		ResultSet rs = stmt
				.executeQuery("Select Fornavn, validFrom, validTo from Person WHERE cpr='1312095098'");
		assertTrue(rs.next());
		...
		rs = stmt
				.executeQuery("Select Fornavn, validFrom, validTo from Person WHERE cpr='1312095098' ORDER BY validFrom");
		assertTrue(rs.next());
		assertEquals("Hjalte", rs.getString("Fornavn"));
		assertEquals("2010-03-15 00:00:00.0", rs.getString("validFrom"));
