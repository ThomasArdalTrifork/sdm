...
import com.trifork.sdm.importer.importers.sor.xml.AddressInformation;
import com.trifork.sdm.importer.importers.sor.xml.HealthInstitutionEntity;
import com.trifork.sdm.importer.importers.sor.xml.InstitutionOwnerEntity;
...
public class SOREventHandler extends DefaultHandler
{
			...
			for (HealthInstitutionEntity hie : curIOE.getHealthInstitutionEntity())
			{
						...
						|| hie.getInstitutionType() == 550671000005100L)
				{
					...
					for (OrganizationalUnitEntity oue : hie.getOrganizationalUnitEntities())
					{
						...
						{
							new SAXException("L푚epraksis skal ikke have en level 2 OrganizationalUnitEntity. SORId=" 
		...
		try
		{
			...
			if (method != null)
			{
				...
				try
				{
					...
					{
						Boolean b = ("true".equalsIgnoreCase(value)) ? new Boolean(true)
								: new Boolean(false);
						method.invoke(object, b);
					}
					else
					{
						logger.error("Unsupported datatype for property " + qName
								+ ", datatype was " + param.getClass().getName());
						throw new Exception("Unsupported datatype for property " + qName
								+ ", datatype was " + param.getClass().getName());
					}
				}
				...
				{
					logger.error(
							"Illegal argument exception on property " + qName + ", method "
									+ object.getClass().getName() + "." + method.getName() + "("
									+ param.getName() + ")", e);
					throw e;
				}
				...
				{
					logger.error(
							"Illegal access exception on property " + qName + ", method "
									+ object.getClass().getName() + "." + method.getName() + "("
									+ param.getName() + ")", e);
					throw e;
				}
				...
				{
					logger.error(
							"Invocation target exception on property " + qName + ", method "
									+ object.getClass().getName() + "." + method.getName() + "("
									+ param.getName() + ")", e);
					throw e;
				}
	...
	public static Calendar toCalendar(String xmlDate) throws java.text.ParseException
	{
		...
		{
			logger.error("Invalid date found in xml. Expected  " + datePattern + " found "
					+ xmlDate, e);
			throw e;
		}
		return cal;
